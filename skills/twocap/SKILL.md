...
name: twocap
description: >-
  Solve captchas (reCAPTCHA v2/v3, Cloudflare Turnstile, hCaptcha, FunCaptcha,
  GeeTest, Amazon WAF, image/text captchas) via the twocap MCP server or CLI,
  which wraps 2captcha.com. Invoke whenever task involves any interaction with
  a captcha or bot-check blocking automation — a page shows "I'm not a robot",
  a scrape/login/form-submit hits reCAPTCHA/Turnstile/hCaptcha, a sitekey needs
  solving, or the twocap/2captcha tools appear in the MCP tool list.
---

# twocap

This tool solves captchas — it does not detect or bypass them on its own. Find the sitekey and
page URL yourself (read the page HTML/DOM for `data-sitekey`, `k=`, or a widget script src), then
hand those values to the tool. Every solve costs real money on the configured 2captcha account —
don't call `captcha_solve` speculatively or wrap it in your own retry loop; the tool already
retries internally (default 5 attempts).
...
5. Optionally call `captcha_report` / `twocap report ID --good|--bad` if the token was accepted or
   rejected downstream — this refunds cost on bad reCAPTCHA v3 tokens and improves future solves.
...
real key). See the repo `README.md` for the full configuration table and deployment instructions.
